...
class ProductService():
    ...
    def _product_by_id_and_seller_id(self, product_id):
        product = self.db.query(Product).filter(Product.id == product_id, Product.seller_id == self.seller.id).first()
        if not product:
            raise HTTPException(status_code=404, detail="Product not found")
        return product
    
    def _variant_by_ids(self, product_id, variant_id):
        variant = (self.db.query(ProductVariant)
                .join(Product)
    ...
    def create_variant(self, name, price, product_id, stock, image):
        ...
        self.db.add(variant)
        self.db.flush()
        variant.image = self._create_image_path(variant=variant, product_id=product_id,
                                                variant_id=variant.id, image=image)
        self.db.commit()
    ...
    def change_variant(self, product_id, variant_id, name, price, stock, image):
        ...
        if image: variant.image = self._create_image_path(variant=variant, product_id=product_id,
                                                          variant_id=variant.id, image=image)
        self.db.commit()
        return variant
